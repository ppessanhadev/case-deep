  ...
  <tbody class="[&>tr>td]:p-2 [&>tr>td]:border [&>tr>td]:border-slate-300">
    <tr>
      ...
      <td v-click-outside="closeOptions" class="relative text-center">
        ...
        <ContextMenu v-if="state.options" :close="handleOptions" :delete="() => store.deleteComputer(data.id)" :update="handleUpdate" />
      </td>
    </tr>
  </tbody>

  <ModalFormComputer v-if="state.update" :close="handleUpdate" :data="data" />
</template>
