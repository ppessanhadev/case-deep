<script setup lang="ts">
  ...
  defineProps<{ data: AirConditioner }>();
  ...
  <tbody class="[&>tr>td]:p-2 [&>tr>td]:border [&>tr>td]:border-slate-300">
    <tr>
      ...
      <td v-click-outside="closeOptions" class="relative text-center">
        ...
        <ContextMenu v-if="state.options" :close="handleOptions" :delete="() => store.deleteAirConditioner(data.id)" :update="handleUpdate" />
      </td>
    </tr>
  </tbody>

  <ModalFormAirConditioner v-if="state.update" :close="handleUpdate" :data="data" />
</template>
